...
st.markdown(
    ...
    This page explores whether potent synthetic lethal gene pairs show direct protein–protein interactions (STRING) or enrichment in shared pathways (g:Profiler).<br>
    Results are based on 197 potent SL pairs derived from CNA–CRISPR regression analysis.
    </div>
...
with st.expander("📄 View All Pairs with PPI Annotation"):
...
# --- Section 2: Pathway enrichment barplot ---
...
# --- Section 3: Dot Plot: Gene × Pathway ---
st.subheader("🧬 Gene × Pathway Dot Plot")
top_pathways = top["name"].tolist()
...
sns.heatmap(dot_matrix.T, cmap=sns.color_palette(["white", "#235b91"]), cbar=False, linewidths=0.5, linecolor="gray", ax=ax2)
ax2.set_xlabel("Pathway")
ax2.set_ylabel("Gene")
ax2.set_title("Genes Involved in Top 15 Enriched Pathways")
st.pyplot(fig2)

# --- Section 4: Chord diagram of curated pathways ---
