...
if mode == "T-test + Cohen's d":
    ...
    filtered_stats = filtered_stats[filtered_stats["P-value"] < p_thresh]
else:
    filtered_stats = None
...
def get_image_download_link(fig, filename="plot.png"):
    ...
    return f'<a href="data:image/png;base64,{b64}" download="{filename}">📥 Download plot as PNG</a>'
...
def plot_boxplot(gene, data):
    ...
    sns.boxplot(x="CNA", y="Protein", data=gene_df, palette="Set1", ax=ax)
    ax.set_title(f"Protein Expression vs CNA for {gene}")
    ax.yaxis.set_major_locator(plt.MaxNLocator(6))
    st.pyplot(fig)
    st.markdown(get_image_download_link(fig, f"{gene}_boxplot.png"), unsafe_allow_html=True)
...
def plot_regression(gene, data):
    ...
    st.pyplot(fig)
    st.markdown(get_image_download_link(fig, f"{gene}_regression.png"), unsafe_allow_html=True)
...
if mode == "T-test + Cohen's d":
    if comp_mode == "Single Gene":
        ...
        with col1:
            ...
            st.write(f"**P-value (Amp):** {gene_stats['P-value (Amplification vs Neutral)']:.2e}")
            st.write(f"**Cohen's d (Amp):** {gene_stats['Cohen\'s d (Amplification vs Neutral)']:.3f}")
        with col2:
            st.write(f"**T-stat (Del vs Neutral):** {gene_stats['T-statistic (Deletion vs Neutral)']:.3f}")
            st.write(f"**P-value (Del):** {gene_stats['P-value (Deletion vs Neutral)']:.2e}")
            st.write(f"**Cohen's d (Del):** {gene_stats['Cohen\'s d (Deletion vs Neutral)']:.3f}")
        ...
        for col, g in zip([col1, col2], [gene1, gene2]):
            ...
            col.markdown(f"### 🧬 {g}")
            col.write(f"**T-stat (Amp):** {gene_stats['T-statistic (Amplification vs Neutral)']:.3f}")
            col.write(f"**P-value (Amp):** {gene_stats['P-value (Amplification vs Neutral)']:.2e}")
            col.write(f"**Cohen's d (Amp):** {gene_stats['Cohen\'s d (Amplification vs Neutral)']:.3f}")
            col.write(f"**T-stat (Del):** {gene_stats['T-statistic (Deletion vs Neutral)']:.3f}")
            col.write(f"**P-value (Del):** {gene_stats['P-value (Deletion vs Neutral)']:.2e}")
            col.write(f"**Cohen's d (Del):** {gene_stats['Cohen\'s d (Deletion vs Neutral)']:.3f}")
        plot1, plot2 = st.columns(2)
        with plot1: plot_boxplot(gene1, cnv_prot_df)
        with plot2: plot_boxplot(gene2, cnv_prot_df)
    ...
    if comp_mode == "Single Gene":
        ...
        plot1, plot2 = st.columns(2)
        with plot1: plot_regression(gene1, cnv_prot_df)
        with plot2: plot_regression(gene2, cnv_prot_df)
    ...
    merged["CNA"] = pd.to_numeric(merged["CNA"], errors="coerce")
    merged["Protein"] = pd.to_numeric(merged["Protein"], errors="coerce")
    ...
    if len(merged) < 6:
        ...
        st.pyplot(fig)
        st.markdown(get_image_download_link(fig, f"{gene_cna}_{gene_prot}_advanced_regression.png"), unsafe_allow_html=True)
